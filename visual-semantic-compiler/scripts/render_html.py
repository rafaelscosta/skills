...
def node_text(n):
    lines=wrap_label(n['label']); cx=n['x']+n['width']/2; cy=n['y']+n['height']/2; first=cy-(len(lines)-1)*10
    spans=''.join(f'<tspan x="{cx}" y="{first+i*20}">{esc(line)}</tspan>' for i,line in enumerate(lines))
    return f'<text class="node-label" text-anchor="middle">{spans}</text>'
...
def render_svg(ir,layout):
    ...
    for e in layout['edges']: parts.append(f'<path class="edge edge-{esc(e.get("kind","other"))}" d="{path_d(e["points"])}" marker-end="url(#arrow)"/>')
...
def deliver(ir_path,out_path,layout_path=None):
    ...
    html_bytes=html_doc(ir,layout,semantic_sha,lr).encode('utf-8'); ar=av.validate_bytes(html_bytes,semantic_sha,lr['layout_sha256'])
